...
type AnimOptions struct {
	Pos_offset   skelform.Vec2
	Scale_factor float32
}

func (ao *AnimOptions) Init() {
	ao.Pos_offset = skelform.Vec2{X: 0, Y: 0}
	ao.Scale_factor = 0.25
}

func Animate(screen *ebiten.Image, armature skelform.Armature, texture image.Image, anim_idx int, frame int, anim_options AnimOptions) {
	animated_bones := skelform.Animate(armature, anim_idx, frame)
	tex := ebiten.NewImageFromImage(texture)

	var props []skelform.Bone
...
func Get_frame_by_time(anim skelform.Animation, unix_milli int64, reverse bool) int {
	return skelform.Get_frame_by_time(anim, unix_milli, reverse)
}

func Load(path string) (skelform.Root, image.Image) {
	return skelform.Load(path)
}
